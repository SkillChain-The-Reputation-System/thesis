...
\begin{itemize}
  \item \textbf{Uy tín chuyên môn} $R_{\text{domain}}$ là điểm uy tín phân theo từng lĩnh vực chuyên môn của thử thách. Mỗi người dùng có thể có mức uy tín khác nhau ở từng lĩnh vực tuỳ theo lịch sử hoạt động của họ.
  \item \textbf{Uy tín toàn cục} $R_{\text{global}}$ phản ánh tổng mức độ hành vi của người dùng xoay quanh các thử thách trong toàn bộ hệ thống.
\end{itemize}
...
Mỗi khi người dùng hoàn thành một trong các hoạt động trên, hệ thống sẽ tính toán một \textit{điểm biến thiên hoạt động}, từ đó điều chỉnh điểm uy tín chuyên môn trong lĩnh vực tương ứng và điểm uy tín toàn cục của người dùng .
...
Điểm chất lượng sẽ được so sánh với \textit{ngưỡng điểm đóng góp} $T_{\text{contribution}}$ -- một hằng số nằm trong khoảng $0$ tới $100$ nhằm xác định điểm uy tín của cá nhân sau đó tăng hay giảm --
để tính $\Delta R_{\text{contribution}}$ theo các công thức khác nhau:
...
Độ lệch điểm chất lượng sẽ được so sánh với \textit{ngưỡng điểm kiểm duyệt} $T_{\text{moderation}}$ -- một hằng số nằm trong khoảng $0$ tới $100$ nhằm xác định điểm uy tín của cá nhân sau đó tăng hay giảm --
để tính $\Delta R_{\text{moderation}}$ theo các công thức khác nhau:
...
Điểm giải pháp sẽ được so sánh với \textit{ngưỡng điểm tham gia} $T_{\text{participation}}$ -- một hằng số nằm trong khoảng $0$ tới $100$ nhằm xác định điểm uy tín của cá nhân sau đó tăng hay giảm --
để tính $\Delta R_{\text{participation}}$ theo các công thức khác nhau:
...
Cuối cùng, người tham gia đánh giá sẽ có \textit{điểm đánh giá} $R_{\text{participation}}$.
...
ta sẽ có điểm giải pháp $S_{\text{participation}}$ cuối cùng, từ đó xác định được độ lệch điểm đánh giá $D_{\text{evaluation}}$:

\[D_{\text{evaluation}}=|S_{\text{solution}}-s_{\text{evaluate}}|\]

\subsubsection{Tính điểm biến thiên}

Độ lệch điểm đánh giá sẽ được so sánh với \textit{ngưỡng điểm đánh giá} $T_{\text{evaluation}}$ -- một hằng số nằm trong khoảng $0$ tới $100$ nhằm xác định điểm uy tín của cá nhân sau đó tăng hay giảm --
để tính $\Delta R_{\text{evaluation}}$ theo các công thức khác nhau:
